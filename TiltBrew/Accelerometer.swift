...
final class Accelerometer: ObservableObject {
    ...
    /// Gravity vector: x = left/right tilt, y = forward/back tilt
    /// Values are in g (~0 at rest flat, ranges roughly -1 to 1)
    ...
    /// Roll angle (left-right tilt) normalized to roughly -1...1
    var roll: Double {
        lock.lock()
        defer { lock.unlock() }
        return smoothedX
    ...
    /// Tilt magnitude (how much total tilt)
    var tiltMagnitude: Double {
        lock.lock()
        defer { lock.unlock() }
        return sqrt(smoothedX * smoothedX + smoothedY * smoothedY)
    }
    ...
    func start() {
        ...
        sensorThread?.name = "TiltBrew-Accelerometer"
        sensorThread?.start()
    }
